...
def lookup_township(townships_db: Path, lat: float, lon: float) -> dict:
    """Return {county, district} via admin polygon containment."""
    conn = sqlite3.connect(str(townships_db))
    pt = Point(lon, lat)
    out = {"county": None, "district": None}
    # Try level 8 (縣轄鄉鎮市) and level 7 (直轄市區) first → district.
    # Then level 4 (縣 / 直轄市) → county.
    for lvl in (8, 7):
        rows = conn.execute(
...
def main() -> int:
    ...
    # Tier 1
    t0 = time.time()
    t = lookup_township(townships_db, args.lat, args.lon)
    print(f"Tier 1 (township):  {t['county']} {t['district']}    "
